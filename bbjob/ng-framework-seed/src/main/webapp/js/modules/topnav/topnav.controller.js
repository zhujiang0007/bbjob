(function() {
	...
	function TopNavController($state, $rootScope, $scope, MenuService, $http,toaster,
		...
		$scope.noticeCount = 0;
		$scope.temp = {};
//		MenuService.getRootMenu(onItemsReady);
		$http.get('server/topNav.json').success(onItemsReady);
		...
		function onItemsReady(data, status, headers, config) {
			if (data.statusCode = "200") {
				$scope.menuItems = data.data;
				init();
			} else
				alert(data.statusCode + "  message:" + data.message)
		}
